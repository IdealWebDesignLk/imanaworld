...
* Fix: number columns (Total / Reserved / Available, Total on orders,
  etc.) were right-aligned in the rows but left-aligned in the column
  headings, so the figures sat far from their titles and it was hard to
  tell which number was which. Headings now align with their numbers on
  the admin tables and the vendor dashboard tables.
...
* Fix: the vendor dashboard's Branch stock table overflowed on phone-width
  screens with no visible cue that the Save/Remove actions were still
  reachable by scrolling sideways — confirmed live at 375px, ~217px of the
  table was hidden off-screen. Shrunk the table for narrow screens and
  added a fading edge so it's clear there's more to scroll to. (#56)
...
* Fix: the 0.9.36 "show this vendor's whole catalogue" toggle didn't work
  for a vendor with no branches yet — confirmed live (Bagwai Investment):
  it fell straight through to the pre-existing "add a branch first" empty
  state instead of listing the vendor's products, exactly the situation
  where seeing the catalogue first is most useful. The catalogue view no
  longer requires branches to exist.

= 0.9.36 =
* Admin Stock screen: with one partner selected, added a "Show this
  vendor's whole catalogue" toggle — previously the screen could only ever
  show products already added to Click & Collect, so there was no way to
  tell "this vendor has no more products" apart from "these products just
  aren't in Click & Collect yet". Products not yet added are labelled "Not
  added" rather than hidden entirely. (#54)
...
* Removed the "Choppies" pilot-partner reference from every email — the
  shared header showed "Choppies · Click & Collect" on every customer and
  branch-staff email; now just "Click & Collect".
...
* Reports: added charts throughout the IPN Reports screen — "Orders by
  branch" and "Branch sales performance" are now bar charts, "Express vs
  Standard split" is now a donut chart, and a new daily revenue trend line
  chart covers the whole selected range (7/30/90 days), not just a snapshot.
  All plain inline SVG — no new JS dependency. (#52)
* Reports: added a dedicated Revenue section — total revenue for the
  selected period (with the existing range/branch filters), broken down by
  Standard vs Express, alongside the new trend chart. (#52)
...
* Added a short description to the Daily Digest screen explaining what it's
  for (orders that expired uncollected and may still need a refund).
...
* Fix: the branch staff dashboard's Branch Stock search box had no submit
  button at all, only relying on the Enter key with no visible way to
  trigger a search - unlike the vendor dashboard's otherwise-identical
  Branch Stock search, which already has one. Added a matching Search
  button.
...
* Style the vendor-mismatch notice (#39) to match the plugin's own palette
  instead of WooCommerce's default red banner.
...
* Fix: "Cancel" on the different-vendor notice (#39) linked back to the exact
  page the customer was already on. Nothing was ever added to the cart on
  that path, so the click correctly changed nothing — but a page reloading to
  look identical to before gives no sign the click did anything, which reads
  as a broken button. It now takes the customer to their cart instead, which
  shows their original vendor's items untouched — visible proof rather than a
  silent no-op.
